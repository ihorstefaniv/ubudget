...
export function Checkbox({ checked, onChange, disabled = false, size = "md" }: CheckboxProps) {
  ...
  return (
    <button role="checkbox" aria-checked={checked} onClick={() => !disabled && onChange(!checked)} disabled={disabled}
      className={["rounded flex items-center justify-center border-2 transition-colors shrink-0 focus:outline-none", box, checked ? "bg-orange-400 border-orange-400 text-white" : "border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-800", disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:border-orange-400"].join(" ")}>
...
export function CheckboxRow({ label, desc, checked, onChange, disabled, size }: CheckboxRowProps) {
  return (
    <div className="flex items-center gap-3 cursor-pointer" onClick={() => !disabled && onChange(!checked)}>
