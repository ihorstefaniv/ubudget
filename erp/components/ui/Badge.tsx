...
const PULSE_COLORS: Record<BadgeColor, string> = {
  ...
  blue: "bg-blue-500", purple: "bg-purple-500", amber: "bg-amber-500", neutral: "bg-neutral-400",
};

export function Badge({ children, color = "neutral", pulse = false }: { children: React.ReactNode; color?: BadgeColor; pulse?: boolean }) {
  return (
    <span className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full font-semibold ${BADGE_COLORS[color]}`}>
      {pulse && <span className={`w-1.5 h-1.5 rounded-full ${PULSE_COLORS[color]} animate-pulse`} />}
      {children}
    </span>
  );
}

export function ProgressBar({ value, max, color, height = "sm" }: { value: number; max: number; color?: string; height?: "sm" | "md" }) {
  ...
  return (
    <div className={`w-full bg-neutral-100 dark:bg-neutral-800 rounded-full overflow-hidden ${height === "sm" ? "h-2" : "h-3"}`}>
      <div className="h-full rounded-full transition-all duration-700" style={{ width: `${pct}%`, backgroundColor: color ?? autoColor }} />
    </div>
  );
}
...
export function Spinner({ size = "md", className = "" }: { size?: "sm" | "md" | "lg"; className?: string }) {
  ...
  return <div className={`${sizes[size]} rounded-full border-2 border-orange-400 border-t-transparent animate-spin ${className}`} />;
}
